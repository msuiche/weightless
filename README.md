...
| **DSV4-Vision-Exp TP=2** | both Sparks over dual-rail RoCE | DeepSeek-V4-Flash-Vision-Exp, FP8 original (202 GB) | projective cvec, L10–38 at α=1.0 (the -ffn hook relabel of [GLP-29 Vision-Exp](https://huggingface.co/msuiche/DeepSeek-V4-Flash-Vision-Exp-abliterated-cyber-GLP-29)) | **blocked at boot 2026-09-19** — the 0.25.2 image's DeepseekV4ForCausalLM refuses the checkpoint's `vision.*`/`aligner` weights at load; steering engaged first (α=1.000, 29 layers, both ranks). Lane lands in `recipe/anemll/` (`.env.dsv4vx` + `start-deepseek-v4-flash-visionexp-dspark.sh`, own compose project + remote dir so 0731 survives swaps); NVFP4 flip is one env block (`ENABLE_VLLM_GB10_PATCH=1`) once the quant is staged |
...
| `vllm-plugin/` | the same steering as an installable vLLM plugin (dist `weightless-steer`, `vllm.general_plugins` entry point): one adapter per architecture in `weightless_steer/archs/` shadows the model class in `ModelRegistry` instead of rewriting files in the container — the successor to the hotfix fleet above. `nemotron_h.py` and `glm5next.py` today; glm5next **GPU-validated 2026-09-18** (Modal 4×H100, GLM-5.3-Flash NVFP4, compiled mode — cyber32 31/32 exactly the hotfix reference, benign32 clean, refusal32 2→9/32 vs 1→21/32 hotfix on a stiffer stock baseline; `modal/cloud_serve_glm53.py`, numbers in BENCHMARK.md). Design: `docs/vllm-plugin-design.md`, offline tests need neither GPU nor vLLM |
