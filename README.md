...
| `setup.py` | full-chain setup wizard (TUI or prompts; Python 3.9+, stdlib only — no requirements.txt by design): pick a lane → site values (prefilled from any existing env) → env file → steering validation → ssh deploy (confirm-gated) → omp provider + endpoint tests; also has a diagnose chain (DNS → TCP → HTTP → remote docker/GPU status over ssh, optional boot) for when the endpoint is down |
