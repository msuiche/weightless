#!/usr/bin/env bash
# Vision-Exp variant of start-deepseek-v4-flash-dspark.sh: same Anemll DSV4
# recipe, but its own env file (.env.dsv4vx), compose project
# (deepseek-v4-flash-visionexp) and remote dir (dspark-visionexp) so the 0731
# lane's on-rig files stay intact across swaps. Everything else is identical.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ENV_FILE="${ENV_FILE:-$SCRIPT_DIR/.env.dsv4vx}"
COMPOSE_FILE="${COMPOSE_FILE:-$SCRIPT_DIR/docker-compose.dsv4.yml}"
PROJECT_NAME="${PROJECT_NAME:-deepseek-v4-flash-visionexp}"
...
if [ ! -f "$ENV_FILE" ]; then
  echo "Missing $ENV_FILE. Copy .env.dsv4vx.example to .env.dsv4vx and edit node-specific values." >&2
  exit 1
fi
...
REMOTE_ENV_FILE="$REMOTE_WORKER_DIR/.env.dsv4vx"
...
print_startup_logs() {
  ...
  remote_compose "docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.dsv4.yml logs --since '$since' vllm-dspark" || true
}
...
print_initial_startup_logs() {
  ...
  remote_compose "docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.dsv4.yml logs --tail=100 vllm-dspark" || true
}
...
print_failure_logs() {
  ...
  remote_compose "docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.dsv4.yml logs --since '$since' vllm-dspark" >&2 || true
}
...
remote_compose "NODE_RANK=1 HEADLESS=1 HF_CACHE='$WORKER_HF_CACHE' VLLM_HOST_IP='$WORKER_VLLM_HOST_IP' GPU_MEMORY_UTILIZATION='$GPU_MEMORY_UTILIZATION' WEIGHTLESS_MODEL='$WEIGHTLESS_MODEL' DSPARK_REVISION='${DSPARK_REVISION:-}' ENABLE_VLLM_GB10_PATCH='$ENABLE_VLLM_GB10_PATCH' VLLM_GB10_PATCH_DIR='./vllm_patch_gb10' GB10_HYBRID_NVFP4_M_THRESHOLD='${GB10_HYBRID_NVFP4_M_THRESHOLD:-128}' docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.dsv4.yml up -d"
...
for _ in $(seq 1 "$WAIT_ATTEMPTS"); do
  if curl -fsS --max-time 5 "${AUTH_HEADER_ARGS[@]}" "$API_URL" >/dev/null 2>&1; then
    ...
    compose_base 0 "" ps
    remote_compose "docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.dsv4.yml ps"
    ...
    if [ "${ENABLE_VL_SIDECAR:-0}" = "1" ] && [ -f "$SIDECAR_COMPOSE_FILE" ]; then
      ...
      remote_compose "MASTER_ADDR='$MASTER_ADDR' NODE_RANK=1 HEADLESS=1 HF_CACHE='$WORKER_HF_CACHE' docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.vl-sidecar.yml up -d"
      ...
      if [ "$SIDECAR_READY" = "1" ]; then
        ...
        remote_compose "docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.vl-sidecar.yml logs --tail=80" >&2 || true
      fi
    fi
...
echo "Recent worker logs:" >&2
remote_compose "docker compose -p '$PROJECT_NAME' --env-file .env.dsv4vx -f docker-compose.dsv4.yml logs --tail=120 vllm-dspark" >&2 || true
exit 1
