...
Generic wedge/restart-loop knowledge lives in `../../TROUBLESHOOTING.md`.
Lane-specific notes:

- The peer gate (`docker-compose.dsv4.yml`) takes effect at next container
  recreate — and the start script does NOT sync this file to the nodes;
  copy it over manually when it changes.
- Watchdog + panic sysctls were first armed here on 2026-09-10; the wizard
  now arms them automatically on every deploy (`harden_steps`).
