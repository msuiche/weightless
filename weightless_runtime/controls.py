...
def _alpha_bounds() -> tuple[float, float]:
    """Server policy window for any client-supplied alpha.

    Alpha is the most model-sensitive knob in the program -- the README
    records that a>=2.5 garbles GLM-5.3-Flash, a=1.0 garbles Inkling-Small,
    and a>1 makes refusal worse on the GLM-5.3 743B -- so an unbounded
    per-request alpha lets any client drive a lane out of its calibrated
    range. Every other request dimension already has a server cap
    (WEIGHTLESS_CONTROL_MAX_LAYERS / _MAX_LAYER_ID / _MAX_POSITION /
    _MAX_SEGMENTS, WEIGHTLESS_TRACE_MAX_*); this is the matching one.

    The default window is [0.0, 4.0]: 0.0 keeps the alpha-zero null arm
    (scripts/compare-weightless-alpha-zero.py) reachable, and 4.0 is the
    highest alpha any shipped lane runs (.env.v027.working.example). It
    bounds only client OVERRIDES -- the server's own
    WEIGHTLESS_STEER_ALPHA is never checked against it, so raising a lane's
    static alpha above the window does not need this touched.
    """
    minimum = _policy_float("WEIGHTLESS_CONTROL_MIN_ALPHA", 0.0)
    maximum = _policy_float("WEIGHTLESS_CONTROL_MAX_ALPHA", 4.0)
    if maximum < minimum:
        raise ValueError(
            "WEIGHTLESS_CONTROL_MAX_ALPHA must be >= "
            "WEIGHTLESS_CONTROL_MIN_ALPHA."
        )
    return minimum, maximum


def _bounded_alpha(raw: Any, field: str) -> float:
    """_finite_float plus the server alpha policy window."""
    value = _finite_float(raw, field)
    minimum, maximum = _alpha_bounds()
    if not minimum <= value <= maximum:
        raise ValueError(
            f"{field} must be within the server alpha policy "
            f"[{minimum:g}, {maximum:g}], got {value:g}. Raise "
            f"WEIGHTLESS_CONTROL_MAX_ALPHA / lower "
            f"WEIGHTLESS_CONTROL_MIN_ALPHA to widen it."
        )
    return value
...
def _parse_schedule(raw: Any) -> tuple[WeightlessScheduleSegment, ...]:
    ...
    for index, item in enumerate(raw):
        ...
        if has_constant:
            start_alpha = end_alpha = _bounded_alpha(
            ...
            start_alpha = _bounded_alpha(
                item["start_alpha"], f"{item_field}.start_alpha"
            )
            end_alpha = _bounded_alpha(
...
def resolve_weightless_vllm_xargs(
    ...
    *,
    ...
    if "alpha" in controls:
        structured_alpha = _bounded_alpha(
            controls["alpha"],
