#!/usr/bin/env python3
"""Structural checks on the day-0 VL steering boot hotfix.

Sibling of test-dsv4-hotfix-structure.py, targeting
patches/hotfix-dsv4vl-steering-projective.py — the hotfix for the day-0
DeepSeek-V4 VL image tree (vLLM 0.28.1rc1.dev137+g5ab628dd1). Its injected
code is byte-identical to the Anemll-lane hotfix; only the anchors were
re-verified (the day-0 model.py keeps the same deferred-fold mHC
convention, so the one hook site stays ffn_out_pre_residual).
...
Always-on checks (no reference sources needed):
  ...
  2. the embedded GGUF loader parses and keeps the spec enforcement
     (glp.mode=project, ffn_out_pre_residual, direction.0 rejection);
  ...
  5. the injected blocks are byte-identical to the Anemll-lane hotfix's
     (drift guard for the "same injected code" contract).

With a reference model.py from the day-0 image (arg or
DSV4VL_REFERENCE_MODEL_PY), it also applies the hotfix to a SCRATCH COPY
(never the original) and AST-checks the result. Without one it exits 2
(SKIP) after the always-on checks.

    docker run --rm --entrypoint cat vllm-upstream-vision:k2-topk512 \
      /usr/local/lib/python3.12/dist-packages/vllm/models/deepseek_v4/nvidia/model.py \
      > /tmp/day0-model.py
    python3 tests/structure/test-dsv4vl-hotfix-structure.py /tmp/day0-model.py
...
HOTFIX = REPO / "patches/hotfix-dsv4vl-steering-projective.py"
ANEMLL_HOTFIX = REPO / "patches/hotfix-dsv4-steering-projective.py"

PER_LAYER_TARGETS = ("self._steer_dirs[layer_id]", "_GLP_HOOK_DIRS[layer_id]")
SPEC_TOKENS = ("glp.mode", "ffn_out_pre_residual", "direction.0")
SHARED_BLOCKS = ("MARK", "GGUF_SRC", "MODULE_BLOCK", "INIT_BLOCK", "FORWARD_BLOCK",
                 "ANCHOR_IMPORT", "REPLACEMENT_IMPORT", "ANCHOR_MODULE",
                 "REPLACEMENT_MODULE", "ANCHOR_INIT", "REPLACEMENT_INIT",
                 "ANCHOR_FORWARD", "REPLACEMENT_FORWARD")
...
def main(ref_model: pathlib.Path | None) -> int:
    ...
    # 5. drift guard: injected blocks byte-identical to the Anemll-lane hotfix
    try:
        anemll_blocks = extract_strings(ast.parse(ANEMLL_HOTFIX.read_text()))
        for name in SHARED_BLOCKS:
            check(blocks.get(name) == anemll_blocks.get(name),
                  f"{name} identical to the Anemll-lane hotfix")
    except (OSError, SyntaxError) as exc:
        check(False, "Anemll-lane hotfix readable for the drift guard", str(exc))

    # 6. optional: apply to a scratch copy of the real day-0 model.py
...
if __name__ == "__main__":
    ...
    elif os.environ.get("DSV4VL_REFERENCE_MODEL_PY"):
        ref = pathlib.Path(os.environ["DSV4VL_REFERENCE_MODEL_PY"])
    sys.exit(main(ref))
