"""weightless GLP steering bootstrap for the GLM-5.3-Flash Modal serving lane.

Python's `site` module auto-imports `sitecustomize` from PYTHONPATH at EVERY
interpreter startup. That property is used deliberately: on this day-0 dev
build (vLLM 0.28.1rc1.dev580) the plugin's register() does run through
vllm's own `load_general_plugins()` (proven 2026-09-18 by reproducing the
api_server arg phase on CPU: registry resolves Glm5NextForCausalLM to the
steered class), but its INFO log lines never surface in the serve path —
vllm's dictConfig attaches a handler only to the `vllm` logger
...
in the real boot. Without the "weightless GLP steering active" line there is
no boot evidence, and an unverifiable boot counts as unsteered.
...
Wired in by cloud_serve_glm53.py: the file lands at
