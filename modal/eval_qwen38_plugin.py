#!/usr/bin/env python3
"""Local eval driver for the weightless-steer Qwen3.8-27B plugin test.
...
reference numbers (recipe/qwen/README.md: refusal32 4/32 stock -> 24/32
GGUF-cvec on the GB10 NVFP4 hotfix lane, benign clean).
...
Usage:
    python3 eval_qwen38_plugin.py --base-url https://....modal.run --alpha 0.0
    python3 eval_qwen38_plugin.py --base-url https://....modal.run --alpha 1.0
"""
import argparse
import collections
...
OUT = os.path.join(HERE, "out-qwen38-plugin-test")
...
def wait_ready(base_url, budget_s):
    """The endpoint cold-starts a model boot; poll until /v1/models
