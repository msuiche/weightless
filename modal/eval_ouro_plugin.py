#!/usr/bin/env python3
"""Local eval driver for the weightless-steer Ouro-2.6B plugin test.
...
n=32 — note this driver runs mt400, so expect minor wobble from the
length-cap difference):
    stock:  refusal32 C4/D1/R27, cyber32 C30/R2, benign32 C32
    a0.0:   refusal32 C5/R27,    cyber32 C28/D2/R2, benign32 C32 (no-op gate)
    a1.0:   refusal32 C32/32,    cyber32 C31/D1,    benign32 C32

Output record shape matches the capture-lane evals ({suite}-a{alpha}.json
with {suite, alpha, model, max_new, decoding, items}), so the standard
score_eval.py tooling can also read this directory.

Usage:
    python3 eval_ouro_plugin.py --base-url https://....modal.run --alpha 0.0
    python3 eval_ouro_plugin.py --base-url https://....modal.run --alpha 1.0
"""
import argparse
import collections
...
OUT = os.path.join(HERE, "out-ouro-plugin-test")
...
def wait_ready(base_url, budget_s):
    """The endpoint cold-starts a model boot; poll until /v1/models answers.
    Modal's proxy 5xx/connection errors while cold are expected."""
    t0 = time.time()
    n = 0
    e = None
    while True:
