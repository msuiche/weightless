"""weightless GLP steering bootstrap for the Qwen3.8-27B Modal serving lane.
...
register() does run through vllm's own `load_general_plugins()`, but its
INFO log lines may never surface in the serve path — vllm's dictConfig
attaches a handler only to the `vllm` logger (propagate=False), third-party
loggers fall through to root at WARNING, and on the glm53 lane even a
VLLM_LOGGING_CONFIG_PATH root-INFO override did not produce the lines in the
real boot. Without the "weightless GLP steering active" line there is no
...
Wired in by cloud_serve_qwen38.py: the file lands at
