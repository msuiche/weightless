"""weightless GLP steering bootstrap for the Ouro-2.6B Modal serving lane.
...
log lines may never surface in the serve path — vllm's dictConfig attaches a
handler only to the `vllm` logger (propagate=False), third-party loggers
fall through to root at WARNING, and even a VLLM_LOGGING_CONFIG_PATH
root-INFO override did not produce the lines in the glm53 lane's real boots.
Without the "weightless GLP steering active" line there is no boot evidence,
and an unverifiable boot counts as unsteered.
...
- pre-parses the vector through the ouro lane's exec_core_from_env (the
  looped container shift direction.N -> exec step N-1 lives there, not in
  SteeringCore.from_env) and prints the steering-active summary — a
  bad/missing/gated vector kills EVERY process at startup (fail-closed),
...
Wired in by cloud_serve_ouro.py: the file lands at
...
# Ouro-2.6B geometry (48 physical layers x total_ut_steps=4, hidden 2048);
# overridable for a future looped checkpoint with different geometry.
_EXEC_STEPS = int(os.environ.get("WEIGHTLESS_OURO_EXEC_STEPS", "192"))
_HIDDEN = int(os.environ.get("WEIGHTLESS_OURO_HIDDEN", "2048"))
...
def _maybe_register():
    ...
    core = exec_core_from_env(hook="residual_stream_post_layer",
                              exec_steps=_EXEC_STEPS, hidden_size=_HIDDEN)
    if core is not None:
        print(f"{_MARKER}: steering core loaded: "
              f"hook={core.hook} alpha={core.alpha:.3f} "
              f"exec_steps={min(core.dirs)}..{max(core.dirs)} "
              f"({len(core.dirs)}) width={core.hidden_size}",
