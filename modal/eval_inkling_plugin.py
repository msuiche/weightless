#!/usr/bin/env python3
"""Local eval driver for the weightless-steer Inkling-Small plugin test.
...
Reference row (hotfix lane, 2026-09-01/02, thinking ON, max_tokens 4096 —
this lane runs the ARCH-protocol 400, so steered-arm completions truncate
mid-deliberation; classify() reads whatever text exists): refusal32 0/32
stock (total lockdown) -> 30/32 at alpha 0.25, benign32 30/32. Inkling is
the most dose-sensitive model in the program — alpha 0.5 garbles; the
steered arm is 0.25 and nothing higher, ever.

Usage:
    python3 eval_inkling_plugin.py --base-url https://....modal.run --alpha 0.0
    python3 eval_inkling_plugin.py --base-url https://....modal.run --alpha 0.25
"""
import argparse
import collections
...
OUT = os.path.join(HERE, "out-inkling-plugin-test")
...
def wait_ready(base_url, budget_s):
    """The endpoint cold-starts a 159 GiB model boot; poll until /v1/models
