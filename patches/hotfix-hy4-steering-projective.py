#!/usr/bin/env python3
"""Hotfix: projective activation steering for Tencent Hy4-preview (hy_v4 arch).
...
hy_v4 differences from deepseek_v4 (the arch this was cloned from):

- iHC (identity hyper-connections) instead of DSV4's HC: the layer carries
  the FULL multi-stream state in hidden_states as [tokens, hc_mult=4,
  hidden=6144] and each sub-block's hc .post() merges immediately -- no
  pending combine. The post-layer stream is simply hidden_states after each
  layer() call in HYV4Model.forward, and the apply steers each of the 4
  streams independently with one 6144-wide direction per layer (same as
  DSV4: direction width == hidden_size, never the flattened hc*hidden).
- The model forward loop has no layer index variable, so the apply indexes
  the stack with layer.layer_idx (set per decoder layer from its prefix)
  and skips PPMissingLayer entries. That guard is structural (fixed at
  build), not data-dependent, so it does not change the traced graph.
- No MTP buffer anchor is needed; the init block hangs off the
  num_redundant_experts tail of HYV4Model.__init__.
...
- WEIGHTLESS_STEER_PATH set but the vector file is missing/invalid/non-project:
  exit 1, before the ~30-minute model load.
...
in-place inside the container (called from the entrypoint before
``vllm serve``). Idempotent: re-applying is a no-op once the marker is
present. ``--status`` reports state; ``--check`` validates the vector named
by WEIGHTLESS_STEER_PATH without touching model.py.
"""
import os
...
MARK = "# [steering-hotfix] projective activation steering (hy_v4)"
...
def _read_gguf_cvec(path):
    ...
    The day-0 hy4-preview image does not ship the gguf package, so the
    container is parsed directly. Only what the control-vector format needs:
    metadata scalars/strings and direction.<N> tensor payloads.
    """
    import struct
...
def _load_gguf_control_vector(path: str) -> dict:
    ...
    if hook is not None and hook != "residual_stream_post_layer":
        raise ValueError(
            ...
            f"hook point measured ~9x weaker on the DSV4 reference lane; "
            f"refusing to apply."
        )
...
# Module-level block injected after the deepseek_v2 indexer import.
MODULE_BLOCK = (
    "\n"
    "\n"
    "# ---------------------------------------------------------------------------\n"
    "# Projective activation steering (weightless). [steering-hotfix]\n"
    "#\n"
    "# h <- h - alpha * (h . d_hat) d_hat on the residual stream at chosen layers.\n"
    "# Activation space rather than weight space on purpose: the GLP directions\n"
    "# are derived and validated at this hook, and baking them into the MXFP8\n"
    "# checkpoint would hit the requantisation floor for small coefficients.\n"
    "#\n"
    ...
    "# On the DSV4 reference lane the site mattered about 9x (attention-writer\n"
    "# apply left 34.0% refusal, against 3.8% post-layer).\n"
...
# __init__ tail block + _load_steering method, injected at the end of
# HYV4Model.__init__ (before embed_input_ids).
INIT_BLOCK = '''\
        ...
        # difference is not part of vLLM's compile cache key: on the DSV4
        # lane a compiled artifact from a 29-layer run was reused by a
        # 16-layer run and died with KeyError: 23. A dense stack indexed by
        # layer id keeps the graph identical for every layer set, so only
        # values change.
        ...
        # compile and every later boot silently reuses it. Measured on DSV4:
        # alpha of 1.0, 2.0 and 4.0 produced byte-identical refusal rates. A
        # tensor is read at runtime, so alpha takes effect without recompiling.
        self.register_buffer(
            "_steer_alpha",
    ...
    def _load_steering(self, config, device, dtype) -> None:
        ...
        try:
            ...
            for layer_id in range(config.num_hidden_layers):
                ...
                # per-stream width must fail here, not as an opaque broadcast
                # error at serve time. One 6144 direction per iHC stream.
                ...
                # A previous revision (DSV4 lane) dedented them out, which kept
                # only the final iteration and silently steered one layer
            ...
            logger.error("weightless steering load failed (%s); failing closed", exc)
            raise
'''

# Per-layer apply in the forward loop, appended after the layer() call.
FORWARD_BLOCK = '''\
            ...
            # hidden_states is (tokens, hc_mult, hidden_size) under iHC here,
            # and the ellipsis contracts only the last axis, so the component
            # is removed from each hyper-connection stream independently.
            # The PPMissingLayer guard is structural (fixed at graph build),
            # not data-dependent, so it does not change the traced graph.
