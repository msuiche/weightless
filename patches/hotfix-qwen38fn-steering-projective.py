    ...
    def _load_steering(self, config, device, dtype) -> None:
        ...
        try:
            ...
            for layer_id in range(config.num_hidden_layers):
                ...
                if vec.shape[-1] != config.hidden_size * config.hc_count:
                    raise RuntimeError(
                        f"steering vector layer {layer_id} width "
                        f"{vec.shape[-1]} != "
                        f"{config.hidden_size * config.hc_count} "
                        f"(hc_count*hidden_size)"
                    )
            ...
            logger.error("Qwen3.8-Flash-Next steering load failed (%s); failing closed", exc)
            raise
'''
