#!/usr/bin/env python3
"""Hotfix: projective activation steering (DSpark) for the DeepSeek-V4
day-0 VL image.

Same injected code as patches/hotfix-dsv4-steering-projective.py (the
Anemll 0.1.1 / vLLM 0.25.2 lane), re-anchored and re-verified against the
day-0 tree: vLLM 0.28.1rc1.dev137+g5ab628dd1 (the
``vllm/vllm-openai:deepseekv4-flash-vision`` image and derivatives such as
``vllm-upstream-vision:k2-topk512``). The four anchors match that tree
verbatim and each is unique; the VL wrapper's construction path is covered
because ``DeepseekV4ForConditionalGeneration`` builds its language model
with ``init_vllm_registered_model(..., architectures=["DeepseekV4ForCausalLM"])``,
whose ``.model`` is the same ``DeepseekV4Model`` this file patches.
...
post/pre call). layers/alpha/vector gated by env:
    ...
    WEIGHTLESS_STEER_ALPHA   float, default 1.0 in code; the Vision-Exp
                               GLP-29 lane pins 1.0 (the vector's alpha_default)
    WEIGHTLESS_STEER_LAYERS  optional comma list restricting layer ids

Hook-site verification on THIS tree (2026-09-21, source read at
5ab628dd1): ``DeepseekV4DecoderLayer.forward`` ends ``x = self.ffn(...)``;
``return x, residual, post_mix, res_mix`` — the FFN write leaves the layer
UNFOLDED, and the next layer's ``mhc_fused_post_pre_tilelang`` performs
the fold. The model loop's ``hidden_states`` between iterations is
therefore the pending FFN write, the same deferred-fold convention the
Anemll 0.25.2 tree has (there verified by shape probe). The actual
residual streams are only reconstructed at aux-capture and after the last
layer via ``mhc_post_tilelang``. So the one implemented hook stays
``ffn_out_pre_residual``, a ``residual_stream_post_layer`` vector file is
the mismatch case (fails closed), and the matching vector for the
Vision-Exp lane is the ``-ffn`` relabel of the GLP-29 file — the original
residual-site GGUF is refused by the loader below. Vectors captured at
the residual and applied here are transferred vectors: legal, but
``glp.derived_at`` must then differ from ``glp.hook_point`` and the
loader warns loudly.

Loader/patcher mechanics are identical to the Anemll-lane file (embedded
GGUF v3 reader — the day-0 image also lacks the ``gguf`` package —,
glp.mode enforced, direction.0 rejected, layer-id cross-check, dense
zero-padded _steer_stack, unconditional per-layer apply, alpha as a
tensor buffer; the injected comments document which of those were
measured failure modes on the v0.27 line).
